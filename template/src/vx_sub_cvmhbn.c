/** 
    vx_sub_%%cvmhbn%.c - Query interface to GoCAD voxet volumes and GTL. Supports
...
/* cell size */
  ...
  if(%%cvmhbn%_debug) { fprintf(stderr,"using HR VP file ..%s\n\n",p_vp63_basin.FN); }
...
if(%%cvmhbn%_debug) {fprintf(stderr,"using HR TAG file..%s\n\n",p_tag61_basin.FN); }
...
if(%%cvmhbn%_debug) {fprintf(stderr,"using HR VS file..%s\n\n",p_vs63_basin.FN); }
...
void vx_config(char **key, char **val, int len) {
  strcpy(key[0],"interp");
  if(no_interp) {
...
/******/
  ...
  if (entry->coor_utm[1] < 10000000) {
    ...
    if(enhanced == True) {
      ...
      if(%%cvmhbn%_debug) { fprintf(stderr," FOUND cvmh surface -- %lf\n", surface); }
      if (surface < -90000.0) {
...
if(_debug) fprintf(stderr," === POST >>> depth %lf surface %lf utm2 %lf coor %lf \n", depth, surface, entry->coor_utm[2], entry->coor[2]);
    ...
    if(%%cvmhbn%_debug) { fprintf(stderr,"Look into (HR)>>>>>> entry->coor(%lf %lf %lf)\n",
                                                        entry->coor[0], entry->coor[1], entry->coor[2]); }
...
//--  if it turns out to be do_bkg, that means return NO DATA 
  ...
  if(%%cvmhbn%_debug) { fprintf(stderr,"   DONE(HR)>>>>>> j(%d) gcoor(%d %d %d) vp(%f) vs(%f) rho(%f)\n",j, gcoor[0], gcoor[1], gcoor[2], entry->vp, entry->vs, entry->rho); }
...
if(%%cvmhbn%_debug) fprintf(stderr,"DONE --- vx_getcoord_private\n");
...
{
if(%%cvmhbn%_debug) fprintf(stderr,"CALLING -- vx_getsurface_private\n");
  int gcoor[3];
  double SP[2],SPUTM[2];
  int j;
  vx_entry_t entry;
...
// go down to see where the actual surface for this point is
	  ...
	  if ((entry.vp < 0.0) || (entry.vs < 0.0)) {
	      ...
	      *surface = p0_NO_DATA_VALUE;
              flag=1;
if(%%cvmhbn%_debug) fprintf(stderr,"==SURFACE: HIT the bottom set to NO_DATA_VALUE\n");
              } else { 
...
if(_debug) fprintf(stderr,"==SURFACE: LOOPING is over %lf\n", entry.coor[2]);
  ...
  if (do_bkg) {
    *surface = p0_NO_DATA_VALUE;
if(%%cvmhbn%_debug) fprintf(stderr,"SURFACE: do_bkg set to NO_DATA_VALUE\n");
  }
